...
void JfdaLossLayer<Dtype>::Forward_cpu(
  ...
  int n1, n2, n3;//, n4;
  n1 = n2 = n3 =0;// n4 = 0;
  const Dtype* label_data = bottom[3]->cpu_data();//5
  const int batch_size = bottom[3]->num();//5
  ...
  neg_acc /= n1;
  pos_acc /= n2;
  top[2]->mutable_cpu_data()[0] = neg_acc;//3
  top[3]->mutable_cpu_data()[0] = pos_acc;//4
  ...
  const Dtype* bbox_target_data = bottom[2]->cpu_data() + bottom[2]->offset(n1);//3,3
